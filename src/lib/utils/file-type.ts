...
const CATEGORY_EXTENSION_SETS: Readonly<Record<Exclude<FileCategoryId, 'all' | 'other'>, ReadonlySet<string>>> = {
  video: new Set(CATEGORY_EXTENSIONS.video),
  audio: new Set(CATEGORY_EXTENSIONS.audio),
  document: new Set(CATEGORY_EXTENSIONS.document),
  archive: new Set(CATEGORY_EXTENSIONS.archive),
  aiModel: new Set(CATEGORY_EXTENSIONS.aiModel),
  image: new Set(CATEGORY_EXTENSIONS.image),
};
...
export function category(name: string): FileCategoryId {
  const fileExtension = extension(name);
  // The prebuilt map keeps high-volume filtering at one lookup per row.
  return EXTENSION_CATEGORIES.get(fileExtension) ?? FILE_CATEGORY_IDS.other;
}
export function categoryCounts(names: readonly string[]): Record<FileCategoryId, number> {
...
export function descriptor(name: string): FileTypeDescriptor {
  const fileExtension = extension(name);
  const kind = visualKind(fileExtension);
  return {
...
function visualKind(extension: string): FileVisualKind {
